  ...
  return (
    <group rotation={[0.15, -0.5, 0]}>
      <Float speed={1.2} rotationIntensity={0.08} floatIntensity={0.25}>
        ...
        <mesh position={[0, 0.12, -0.85]}><sphereGeometry args={[0.09, 32, 32]} /><meshStandardMaterial color="#33e6ff" emissive="#33e6ff" emissiveIntensity={1.5} /></mesh>
      </Float>
    </group>
  );
}
...
export function Viewport() {
  return (
    <div className="relative h-full min-h-[520px] overflow-hidden rounded-3xl border border-forge-line bg-gradient-to-br from-forge-panel2 to-[#0b1018] shadow-glow">
      ...
      <div className="absolute bottom-5 left-5 flex gap-2 rounded-2xl border border-forge-line bg-black/35 p-2 backdrop-blur">{['Select','Move','Rotate','Scale','UV','Wire'].map(tool => <button key={tool} className="rounded-xl px-3 py-2 text-xs text-slate-300 hover:bg-white/10">{tool}</button>)}</div>
    </div>
  );
}
